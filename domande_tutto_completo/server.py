...
SURVEY_TYPES = ("worker", "employer")
QUESTION_OPTIONS_BY_SURVEY = {
    "worker": {
        "motivation": ("<16", "16-18", "19-22", ">22"),
        "availability": ("SI'", "NO"),
        "next_step": ("SI'", "NO"),
        "monthly_frequency": ("1", "2-3", "4 O PIU'"),
        "app_usage": ("SI'", "NO"),
        "safety_importance": ("MOLTO", "ABBASTANZA", "POCO"),
        "commission_range": ("1-2%", "3-4%", "5-6%", "7-10%"),
    },
    "employer": {
        "motivation": ("<18", "18-30", "31-50", ">50"),
        "availability": ("SI'", "NO"),
        "next_step": ("SI'", "NO"),
...
def build_record(payload: dict) -> dict:
    ...
    survey_type = payload.get("surveyType", "worker")
    if survey_type not in SURVEY_TYPES:
        raise ValueError("Tipo di questionario non valido.")

    answers = payload.get("answers")
    if not isinstance(answers, dict):
        raise ValueError("Il campo answers e' obbligatorio.")

    expected_options = QUESTION_OPTIONS_BY_SURVEY[survey_type]
    expected_keys = tuple(expected_options.keys())
    cleaned_answers = {}
    for key in expected_keys:
        value = answers.get(key)
        if value not in expected_options[key]:
...
def main() -> None:
    ...
    print(f"Archivio risposte: {RESPONSES_PATH}")
